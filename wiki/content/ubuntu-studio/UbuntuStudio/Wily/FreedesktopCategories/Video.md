# UbuntuStudio/Wily/FreedesktopCategories/Video - Ubuntu Wiki

Source: https://wiki.ubuntu.com/UbuntuStudio/Wily/FreedesktopCategories/Video

## [Video](https://wiki.ubuntu.com/UbuntuStudio/Wily/FreedesktopCategories/Video)

[https://help.ubuntu.com/community/UbuntuStudio](https://help.ubuntu.com/community/UbuntuStudio)

[/UbuntuStudio](https://wiki.ubuntu.com/UbuntuStudio)

[https://help.ubuntu.com/community/UbuntuStudio/Community](https://help.ubuntu.com/community/UbuntuStudio/Community)

| Home | Testing | PR & Support | Artwork | Packaging/Development | Documentation | Organization |
| --- | --- | --- | --- | --- | --- | --- |

**[Home](https://wiki.ubuntu.com/UbuntuStudio)**

**[Testing](https://wiki.ubuntu.com/UbuntuStudio/Testing)**

**[PR & Support](https://wiki.ubuntu.com/UbuntuStudio/PublicRelations)**

**[Artwork](https://wiki.ubuntu.com/UbuntuStudio/Artwork)**

**[Packaging/Development](https://wiki.ubuntu.com/UbuntuStudio/Development)**

**[Documentation](https://wiki.ubuntu.com/UbuntuStudio/Documentation)**

**[Organization](https://wiki.ubuntu.com/UbuntuStudio/Organization)**

Package lists are grouped by release pocket

Contents

1. [Universe](https://wiki.ubuntu.com/UbuntuStudio/Wily/FreedesktopCategories/Video#Universe)

When checking for which categories a desktop file belongs to, use this page as reference - [UbuntuStudio/FreedesktopCategories](https://wiki.ubuntu.com/UbuntuStudio/FreedesktopCategories).

# Universe

| Package Name | Description | Desktop File | Current FDC | Suggested FDC | Bug |
| --- | --- | --- | --- | --- | --- |
| aegisub | advanced subtitle editor | aegisub.desktop | AudioVideo ; AudioVideoEditing ; | Video |  |
| bino | 3D video player | bino.desktop | AudioVideo ;Player;Video; | OK |  |
| freetuxtv | Internet television and radio player | freetuxtv.desktop | AudioVideo ;Video;TV; | Player |  |
| gmlive | live video frontend for mplayer | gmlive.desktop | AudioVideo ; | Player |  |
| gnome-media-player | A simple media player for GNOME | gnome-media-player.desktop | AudioVideo ;GTK; |  |  |
| imagination | DVD slide show maker | imagination.desktop | AudioVideo ; |  |  |
| mediaplayer-app | Ubuntu Media player | mediaplayer-app.desktop |  |  |  |
| minitube | Native YouTube client | minitube.desktop | AudioVideo ;Player;Qt; |  |  |
| mpv | video player based on MPlayer/mplayer2 | mpv.desktop | AudioVideo ;Audio;Video;Player;TV; |  |  |
| nomnom | download videos from Youtube and other similar video websites | nomnom.desktop | AudioVideo ; |  |  |
| smtube | YouTube videos browser | smtube.desktop | Qt;KDE; AudioVideo ;Player;Video; |  |  |
| vlc | multimedia player and streamer | vlc.desktop | AudioVideo ;Player;Recorder; |  |  |
| vokoscreen | easy to use screencast creator | vokoscreen.desktop | AudioVideo ;Recorder; |  |  |

Package Name

Description

Desktop File

Current FDC

Suggested FDC

Bug

[aegisub](https://launchpad.net/ubuntu/wily/+package/aegisub)

advanced subtitle editor

aegisub.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);[AudioVideoEditing](https://wiki.ubuntu.com/AudioVideoEditing);

Video

[bino](https://launchpad.net/ubuntu/wily/+package/bino)

3D video player

bino.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Player;Video;

OK

[freetuxtv](https://launchpad.net/ubuntu/wily/+package/freetuxtv)

Internet television and radio player

freetuxtv.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Video;TV;

Player

[gmlive](https://launchpad.net/ubuntu/wily/+package/gmlive)

live video frontend for mplayer

gmlive.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);

Player

[gnome-media-player](https://launchpad.net/ubuntu/wily/+package/gnome-media-player)

A simple media player for GNOME

gnome-media-player.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);GTK;

[imagination](https://launchpad.net/ubuntu/wily/+package/imagination)

DVD slide show maker

imagination.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);

[mediaplayer-app](https://launchpad.net/ubuntu/wily/+package/mediaplayer-app)

Ubuntu Media player

mediaplayer-app.desktop

[minitube](https://launchpad.net/ubuntu/wily/+package/minitube)

Native [YouTube](https://wiki.ubuntu.com/YouTube) client

minitube.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Player;Qt;

[mpv](https://launchpad.net/ubuntu/wily/+package/mpv)

video player based on MPlayer/mplayer2

mpv.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Audio;Video;Player;TV;

[nomnom](https://launchpad.net/ubuntu/wily/+package/nomnom)

download videos from Youtube and other similar video websites

nomnom.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);

[smtube](https://launchpad.net/ubuntu/wily/+package/smtube)

[YouTube](https://wiki.ubuntu.com/YouTube) videos browser

smtube.desktop

Qt;KDE;[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Player;Video;

[vlc](https://launchpad.net/ubuntu/wily/+package/vlc)

multimedia player and streamer

vlc.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Player;Recorder;

[vokoscreen](https://launchpad.net/ubuntu/wily/+package/vokoscreen)

easy to use screencast creator

vokoscreen.desktop

[AudioVideo](https://wiki.ubuntu.com/AudioVideo);Recorder;

UbuntuStudio/Wily/FreedesktopCategories/Video (last edited 2015-07-13 20:19:37 by c83-251-51-14)
