# UbuntuStudio/DevEvents/Development - Ubuntu Wiki

Source: https://wiki.ubuntu.com/UbuntuStudio/DevEvents/Development

## [Development](Development)

[UbuntuStudio](../../../../../help)

[/UbuntuStudio](..)

[UbuntuStudio/Community](../../../../../help/content/community/UbuntuStudio--Community)
**[Home](..)**

**[Testing](../Testing)**

**[PR & Support](../PublicRelationsDocumentation)**

**[Artwork](../Artwork)**

**[Packaging/Development](../Development)**

**[Documentation](../Documentation)**

**[Organization](../Organization)**

## Tasks during development
|  |  |
| --- | --- |
| week 5-17 | Implement features as defined in blueprints until FeatureFreeze |
| week 5-20 | User interface development continues until UserInterfaceFreeze |
| week 5-21 | Documentation development confinues until DocumentationStringFreeze |
| week 18-25 | Bug-fixing of added features until FinalFreeze |

week 1-4

Preliminary development before [FeatureDefinitionFreeze](https://wiki.ubuntu.com/FeatureDefinitionFreeze).

week 5-17

Implement features as defined in [blueprints](../Blueprints) until [FeatureFreeze](https://wiki.ubuntu.com/FeatureFreeze)

week 5-20

User interface development continues until [UserInterfaceFreeze](https://wiki.ubuntu.com/UserInterfaceFreeze)

week 5-21

Documentation development confinues until [DocumentationStringFreeze](https://wiki.ubuntu.com/DocumentationStringFreeze)

week 18-25

Bug-fixing of added features until [FinalFreeze](https://wiki.ubuntu.com/FinalFreeze)

## Obligatory tasks

### week 1-4

- [ubuntustudio-meta](../Source/ubuntustudio-meta) - change update.cfg to use latest seeds as soon as archive opens

[ubuntustudio-meta](../Source/ubuntustudio-meta) - change update.cfg to use latest seeds as soon as archive opens

UbuntuStudio/DevEvents/Development (last edited 2015-04-26 14:12:04 by h-141-65)
